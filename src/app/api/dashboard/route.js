...
export async function GET() {
  try {
    ...
    const [stats, goals, tasks, english, career, achievements] = await Promise.all([
      prisma.userstats.findUnique({ where: { userId } }),
      prisma.goal.findMany({ where: { userId }, orderBy: { id: "desc" } }),
      prisma.task.findMany({ where: { userId }, orderBy: { id: "desc" } }),
      prisma.englishpractice.findMany({ where: { userId }, orderBy: { createdAt: "desc" } }),
      prisma.careerprogress.findMany({ where: { userId }, orderBy: { updatedAt: "desc" } }),
      prisma.userachievement.findMany({ where: { userId } }),
    ]);

    const totalEnglishMinutes = english.reduce((total, item) => total + item.minutes, 0);
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        goals,
        recentTasks,
        english: { totalMinutes: totalEnglishMinutes, recent: english.slice(0, 5) },
        career,
        achievements,
      },
    });
